//set dimensions
var urlMapa = "https://raw.githubusercontent.com/Juancruzd/mexicovid19/master/assets/geojson/mexico.json";
var now = new Date(); 
var day = ("0" + now.getDate()).slice(-2);
var month = ("0" + (now.getMonth() + 1)).slice(-2); 
var today = (day)+"-"+(month)+"-"+now.getFullYear() ;  
var urldailyreport="https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/"+today+".csv";
//set dimensions
var w = 700;
var h = 400;
var w_full = w, h_full = h;
if (w > $(window).width()) {
    w = $(window).width();
    h = w / 1.75;
}  
...
//load GeoJson data
d3.json(urlMapa, function (json) { 
    d3.csv(urldailyreport, function (data) { 
    var states=[];
    var largo = data.length;
    var tope = largo - 1;
    for (var i = 1; i < tope; i++) {
        if (data[i]["Country_Region"] == "Mexico") { 
             states.push(data[i]);
        }
    }  
    ...
    for (var j = 1; j < json.features.length-1; j++) { 
        ...
        json.features[j].properties.Confirmed=states[position].Confirmed;
        json.features[j].properties.Deaths=states[position].Deaths;
        json.features[j].properties.Recovered=states[position].Recovered;
        json.features[j].properties.Last_Update=states[position].Last_Update;
        json.features[j].properties.Active=states[position].Active
    ...
    var color = d3.scaleQuantize()
        .domain(d3.extent(json.features, function (d) { return d.properties.Confirmed }))
        .range(palette); 
    ...
    mapSvg.selectAll("path")
        ...
        .append("path")
        .attr("d", path)
        .attr('fill', function (d) { return color(d.properties.Confirmed) })
        .on("mouseover", function (d) {


            //d.properties.Last_Update.toLocaleString('en-US', { hour12: false })
            d.properties.Last_Update = new Date(d.properties.Last_Update);
            ...
            navMap.html("<h6>" + d.properties.name + "</h6>" +
                "<p class='text-danger'> Totales: " + d.properties.Confirmed + "</p>" +  
                "<p class='text-primary'> Defunciones: " + d.properties.Deaths + "</p>" +
                "<p><small>Actualizado el: <br>" + formatDay(d.properties.Last_Update) + "/" + formatMonth(d.properties.Last_Update) +
                " @ " + formatHour(d.properties.Last_Update) + ":" + formatMin(d.properties.Last_Update) + "</small></p>")
