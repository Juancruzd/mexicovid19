...
d3.csv(url, function (data) {
    ...
    for (var i = 1; i < tope; i++) {
        if (data[i]["Country/Region"] == "Mexico") {
            for(var j=data.columns.length - 1 ; j>4 ;j--) {
                var uno=Number(data[i][data.columns[j]]);
                var dos=Number(data[i][data.columns[j-1]]);
                var nuevos=uno-dos;
                //console.log("Nuevos: " + nuevos.toLocaleString('es-us'));
                //console.log("Anteriores: " + y.toLocaleString('es-us'));
                //console.log("Fecha: "+data.columns[j]+"   Casos: "+nuevos); 
                data1.push({Fecha:data.columns[j].toString(), Nuevos_JH: Number(nuevos)});
    ...
    var y = d3.scaleLinear()
        .range([h, 0])//height - 10
        .domain([0, 1.1 * data1[tope].Nuevos_JH])//1.1*d3.max(data, function(d) {return d.Nuevos_abiertos;})])
    svgT.append("g")
        .call(d3.axisLeft(y));
